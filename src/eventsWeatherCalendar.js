...
if (city_name == null) {
	Safari.open("scriptable:///run/getLocation");
}
...
////////////////////////////
			...
			//RIGHT - "{SFSymbol Current Weather} [Current Weather]"
...
function testMode (input) {
	...
	//set predicted weather
	let next_hour_temp_icon = "arrowtriangle.up.fill";
	let next_hour_temp_color = "red";
	
	if (input == "off") {
		const today = weatherJSON.list[0];

		//get current high, low temperatures
		current_low = Math.round(today.main.temp_min);
		current_high = Math.round(today.main.temp_max);
		
		//get current temperature, wind speed, and humidity
		current_temp = Math.round(today.main.temp);
		current_wind_speed = Math.round(today.wind.speed);
		...
		//get current weather description (ex: rainy, cloudy, etc)
		current_weather_description = (today.weather[0].main).toLowerCase();
		
			//if current weather is cloudy then specify type of cloudy
			if (current_weather_description === "clouds") {
				current_weather_description = today.weather[0].description;
			} 
			
			//find correct day/night weather emoji
			let sunsetTodayMS = weatherJSON.city.sunset * 1000; //in ms; today
